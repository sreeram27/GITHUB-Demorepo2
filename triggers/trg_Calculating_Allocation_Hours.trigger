trigger trg_Calculating_Allocation_Hours on Allocation_Details__c (after Insert,after update) {
    ...
    Set<Id> alcIds = new Set<Id>();
        ...
        for(Allocation_Details__c ad: Trigger.new){
           /*  code need's to have Check IsInsert vs IsUpdate
           Trigger.OldMap: Trigger.oldMap returns map of old records which are updated with new values. 
            These List of records already there in Database. Trigger.oldmap available in Before update, after update, Before Delete and After Delete triggers.*/
            ...
            if(ad.Vacation_Hours__c != null && ad.Vacation_Hours__c >0) {
                   alcIds.add(ad.Id);               
